...
function toggleTheme() {
  ...
  setTheme(newTheme);
  showToast(
    `Đã chuyển sang ${newTheme === "dark" ? "chế độ tối" : "chế độ sáng"}`,
    "success"
  );
}
...
function initUserInfo(user) {
  ...
  if (welcomeH1) {
    welcomeH1.textContent = `Chào mừng trở lại, ${user.name
      .split(" ")
      .pop()}! 👋`;
  }
}
...
function renderMembers(filter = "all") {
  ...
  if (filter !== "all") {
    filteredMembers = members.filter(
      (m) => getMemberStatus(m.endDate) === filter
    );
  }
...
function createMemberRow(member) {
  ...
  return `
    ...
    member.name
            ...
            <td><span class="status-badge ${status}">${getStatusText(
    status
  )}</span></td>
            <td>
...
function renderExpiringMembers() {
  ...
  container.innerHTML = expiringMembers
    .map((member) => {
      const avatarUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(
        member.name
      ...
      const daysText =
        daysLeft < 0
          ? `Hết hạn ${Math.abs(daysLeft)} ngày`
          : `Còn ${daysLeft} ngày`;
